/* ============================================================
   LI & FUNG HOSPITALITY — Hotel Supplies Catalog Data
   ------------------------------------------------------------
   HOW TO UPDATE (no coding needed):

   1. PRODUCTS (服装/其它展示产品):
      - Put your image file into the folder:  images/uniform/
      - Copy one of the blocks below inside "products: [ ... ]"
        and change the fields:
          name        — product name (English)
          category    — MUST match a subcategory "en" below
                        (currently only "Hotel Uniform" shows products)
          image       — "images/uniform/your-photo.jpg"
          description — one short line
          specs       — key/value pairs shown in the detail view
      - To remove a product, delete its block.

   2. CATEGORIES (品类目录):
      - Listed below in "categories". Only edit text if a name changes.
      - When a new category is ready to show products, set
          live: true
        and add products with that category name.

   Keep the commas and quotes exactly as shown.
   ============================================================ */

const CATALOG = {

  company: {
    name: "LI & FUNG HOSPITALITY",
    tagline: "Your Trusted Hotel Supplies Partner",
    intro:
      "LI & FUNG HOSPITALITY is a one-stop sourcing partner for the hospitality industry, " +
      "covering hotel uniforms, F&B equipment, guestroom supplies, " +
      "operations essentials and more — from standard items to fully " +
      "custom-made solutions.",
    emails: ["JimiLee@lifung.com", "JohnWu@lifung.com"],
  },

  categories: [
    {
      en: "Hotel Uniform",
      zh: "酒店服装",
      live: true,                       // full product showcase
      blurb: "Professional uniforms for every hotel role — front office, F&B, housekeeping, kitchen, spa and management. Custom designs, fabrics and branding available.",
      subs: ["Hotel Uniform"],
    },
    {
      en: "F&B Equipment & Supplies",
      zh: "餐饮用品与设备",
      live: false,
      blurb: "Complete food & beverage service solutions, from buffet setup to tableware.",
      subs: [
        { name: "Buffet Set UP", image: "images/ose/fnb/buffet-set-up.jpeg" },
        { name: "Case Iron Pot", image: "images/ose/fnb/case-iron-pot.jpeg" },
        { name: "Chinaware", image: "images/ose/fnb/chinaware.jpeg" },
        { name: "F&B Linen", image: "images/ose/fnb/fnb-linen.jpeg" },
        { name: "Glass Plat & Lazy Susan", image: "images/ose/fnb/glass-plat-lazy-susan.jpeg" },
        { name: "Glassware", image: "images/ose/fnb/glassware.jpeg" },
        { name: "Kitchen Utensil", image: "images/ose/fnb/kitchen-utensil.jpeg" },
        { name: "Melamine", image: "images/ose/fnb/melamine.jpeg" },
        { name: "Tableware & Chafing Dish", image: "images/ose/fnb/tableware-chafing-dish.jpeg" },
      ],
    },
    {
      en: "Guestroom Equipment & Supplies",
      zh: "客房用品与设备",
      live: false,
      blurb: "Everything for the guestroom — linen, minibar, safes and custom amenities.",
      subs: [
        { name: "Linen", image: "images/ose/guestroom/linen.jpeg" },
        { name: "Minibar & Safe", image: "images/ose/guestroom/minibar-safe.jpeg" },
        { name: "Rooms Custom Made Amenity", image: "images/ose/guestroom/custom-made-amenity.jpeg" },
      ],
    },
    {
      en: "Operations & Back-of-House",
      zh: "运营设备与后勤",
      live: false,
      blurb: "Keep operations running smoothly — trolleys, appliances, mats and cleaning tools.",
      subs: [
        { name: "Buggy", image: "images/ose/operations/buggy.jpeg" },
        { name: "Electrical Appliance", image: "images/ose/operations/electrical-appliance.jpeg" },
        { name: "Floor Mat", image: "images/ose/operations/floor-mat.jpeg" },
        { name: "Trolley & Cleaning Tools", image: "images/ose/operations/trolley-cleaning-tools.jpeg" },
      ],
    },
    {
      en: "Systems & Security",
      zh: "系统与安防",
      live: false,
      blurb: "Door lock systems, fragrance solutions and key card consumables.",
      subs: [
        { name: "Door Locker System", image: "images/ose/security/door-locker-system.jpeg" },
        { name: "Fragrance", image: "images/ose/security/fragrance.jpeg" },
        { name: "Key Card & Other Wood Consume Items", image: "images/ose/security/key-card-wood-consume-items.jpeg" },
      ],
    },
    {
      en: "General Consumables",
      zh: "通用耗材",
      live: false,
      blurb: "Daily consumables and disposables for hotel operations.",
      subs: [
        { name: "Consumable", image: "images/ose/consumables/consumable.jpeg" },
      ],
    },
  ],

  /* -------- UNIFORM SERIES -----------------------------------
     Each Case collection belongs to one series below.
     ------------------------------------------------------------ */
  uniformSeries: [
    {
      id: "readymade",
      title: "Readymade Uniform",
      blurb: "Ready-to-order uniform styles for polished, professional hotel teams.",
    },
    {
      id: "customize",
      title: "Customize Uniform",
      blurb: "Custom-designed uniform collections tailored to each venue, service concept and brand.",
    },
  ],

  /* -------- COLLECTIONS (uniform cases) ---------------------
     Each collection groups products under its own header.
     To add a case: copy a block, give it a new "id", and set
     collection: "<same id>" on its products below.
     ------------------------------------------------------------ */
  collections: [
    {
      id: "case1",
      series: "readymade",
      title: "Front House Collection",
      blurb: "Tailored suits and blazers for front desk, casino floor and premium F&B service teams.",
    },
    {
      id: "case2",
      series: "customize",
      title: "Chinese Restaurant Collection",
      blurb: "Modern oriental silhouettes with bold color blocking — designed for signature Chinese dining venues.",
    },
    {
      id: "case3",
      series: "customize",
      title: "All-Day Dining Service Collection",
      blurb: "Warm earth-tone service sets with aprons for restaurants, cafés and banquet service.",
    },
    {
      id: "case5",
      series: "readymade",
      title: "Resort & Leisure Service Collection",
      blurb: "Polished, relaxed uniforms for resort reception, golf, poolside and casual dining service.",
    },
    {
      id: "housekeeping",
      series: "readymade",
      title: "House Keeping Collection",
      blurb: "Ready-to-wear housekeeping uniforms designed for comfort, mobility and a polished professional appearance.",
    },
    {
      id: "restaurant",
      series: "readymade",
      title: "Restaurant Collection",
      blurb: "Ready-to-wear restaurant uniforms that balance refined presentation, easy care and all-day comfort.",
    },
    {
      id: "case6",
      series: "customize",
      title: "Japanese Restaurant Collection",
      blurb: "Sophisticated host and hostess uniforms created for premium restaurants and destination dining venues.",
    },
    {
      id: "case7",
      series: "customize",
      title: "Nightlife & Lounge Collection",
      blurb: "Statement looks for luxury lounges, clubs, cocktail bars and evening guest experiences.",
    },
  ],

  /* -------- PRODUCTS ----------------------------------------
     Only products whose category is a "live: true" category
     are displayed. "collection" groups the product under a
     case header above. Image path: images/uniform/<case>/<file>
     ------------------------------------------------------------ */
  products: [
    {
      name: "Front House Suit 01",
      category: "Hotel Uniform",
      collection: "case1",
      image: "images/uniform/case1/premium-tweed-suit-women.jpg",
      description: "Burgundy tweed jacket and skirt set with gold trim for VIP relations & executive lounge teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Front House Suit 02",
      category: "Hotel Uniform",
      collection: "case1",
      image: "images/uniform/case1/premium-front-office-suit-men.jpg",
      description: "Burgundy suit with embellished lapels and gold tie for premium front office & guest relations.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Front House Suit 03",
      category: "Hotel Uniform",
      collection: "case1",
      image: "images/uniform/case1/casino-floor-team-suit.jpg",
      description: "One-button suit in light blue for casino marketing & floor teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Front House Suit 04",
      category: "Hotel Uniform",
      collection: "case1",
      image: "images/uniform/case1/front-office-suit-women.jpg",
      description: "Charcoal skirt suit with gold name badge for front office ladies.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Front House Suit 05",
      category: "Hotel Uniform",
      collection: "case1",
      image: "images/uniform/case1/front-office-suit-men.jpg",
      description: "Classic charcoal three-piece style suit for front office & concierge.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Front House Suit 06",
      category: "Hotel Uniform",
      collection: "case1",
      image: "images/uniform/case1/fnb-premium-suit.jpg",
      description: "Burgundy suit with gold tie for premium F&B and captain roles.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Chinese Restaurant Look 01",
      category: "Hotel Uniform",
      collection: "case2",
      image: "images/uniform/case2/chinese-restaurant-look-1.jpg",
      description: "Modern oriental uniform with mandarin collar and contrast palette.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Chinese Restaurant Look 02",
      category: "Hotel Uniform",
      collection: "case2",
      image: "images/uniform/case2/chinese-restaurant-look-2.jpg",
      description: "Modern oriental uniform with mandarin collar and contrast palette.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Chinese Restaurant Look 03",
      category: "Hotel Uniform",
      collection: "case2",
      image: "images/uniform/case2/chinese-restaurant-look-3.jpg",
      description: "Modern oriental uniform with mandarin collar and contrast palette.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Chinese Restaurant Look 04",
      category: "Hotel Uniform",
      collection: "case2",
      image: "images/uniform/case2/chinese-restaurant-look-4.jpg",
      description: "Modern oriental uniform with mandarin collar and contrast palette.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Chinese Restaurant Look 05",
      category: "Hotel Uniform",
      collection: "case2",
      image: "images/uniform/case2/chinese-restaurant-look-5.jpg",
      description: "Modern oriental uniform with mandarin collar and contrast palette.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Chinese Restaurant Look 06",
      category: "Hotel Uniform",
      collection: "case2",
      image: "images/uniform/case2/chinese-restaurant-look-6.jpg",
      description: "Modern oriental uniform with mandarin collar and contrast palette.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Dining Service Look 01",
      category: "Hotel Uniform",
      collection: "case3",
      image: "images/uniform/case3/dining-service-look-1.jpg",
      description: "Earth-tone shirt with full apron for all-day dining service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Dining Service Look 02",
      category: "Hotel Uniform",
      collection: "case3",
      image: "images/uniform/case3/dining-service-look-2.jpg",
      description: "Earth-tone shirt with full apron for all-day dining service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Dining Service Look 03",
      category: "Hotel Uniform",
      collection: "case3",
      image: "images/uniform/case3/dining-service-look-3.jpg",
      description: "Earth-tone shirt with full apron for all-day dining service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Dining Service Look 04",
      category: "Hotel Uniform",
      collection: "case3",
      image: "images/uniform/case3/dining-service-look-4.jpg",
      description: "Earth-tone shirt with full apron for all-day dining service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Dining Service Look 05",
      category: "Hotel Uniform",
      collection: "case3",
      image: "images/uniform/case3/dining-service-look-5.jpg",
      description: "Earth-tone shirt with full apron for all-day dining service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Dining Service Look 06",
      category: "Hotel Uniform",
      collection: "case3",
      image: "images/uniform/case3/dining-service-look-6.jpg",
      description: "Earth-tone shirt with full apron for all-day dining service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Resort Service Look 01",
      category: "Hotel Uniform",
      collection: "case5",
      image: "images/uniform/case5/resort-service-01.jpg",
      description: "Relaxed and refined uniform for resort, leisure and casual hospitality teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Resort Service Look 02",
      category: "Hotel Uniform",
      collection: "case5",
      image: "images/uniform/case5/resort-service-02.jpg",
      description: "Relaxed and refined uniform for resort, leisure and casual hospitality teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Resort Service Look 03",
      category: "Hotel Uniform",
      collection: "case5",
      image: "images/uniform/case5/resort-service-03.jpg",
      description: "Relaxed and refined uniform for resort, leisure and casual hospitality teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Resort Service Look 04",
      category: "Hotel Uniform",
      collection: "case5",
      image: "images/uniform/case5/resort-service-04.jpg",
      description: "Relaxed and refined uniform for resort, leisure and casual hospitality teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Resort Service Look 05",
      category: "Hotel Uniform",
      collection: "case5",
      image: "images/uniform/case5/resort-service-05.jpg",
      description: "Relaxed and refined uniform for resort, leisure and casual hospitality teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Resort Service Look 06",
      category: "Hotel Uniform",
      collection: "case5",
      image: "images/uniform/case5/resort-service-06.jpg",
      description: "Relaxed and refined uniform for resort, leisure and casual hospitality teams.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Japanese Restaurant Look 01",
      category: "Hotel Uniform",
      collection: "case6",
      image: "images/uniform/case6/fine-dining-host-01.jpg",
      description: "Elegant host and hostess uniform for premium restaurant service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Japanese Restaurant Look 02",
      category: "Hotel Uniform",
      collection: "case6",
      image: "images/uniform/case6/fine-dining-host-02.jpg",
      description: "Elegant host and hostess uniform for premium restaurant service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Japanese Restaurant Look 03",
      category: "Hotel Uniform",
      collection: "case6",
      image: "images/uniform/case6/fine-dining-host-03.jpg",
      description: "Elegant host and hostess uniform for premium restaurant service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Japanese Restaurant Look 04",
      category: "Hotel Uniform",
      collection: "case6",
      image: "images/uniform/case6/fine-dining-host-04.jpg",
      description: "Elegant host and hostess uniform for premium restaurant service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Japanese Restaurant Look 05",
      category: "Hotel Uniform",
      collection: "case6",
      image: "images/uniform/case6/fine-dining-host-05.jpg",
      description: "Elegant host and hostess uniform for premium restaurant service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Japanese Restaurant Look 06",
      category: "Hotel Uniform",
      collection: "case6",
      image: "images/uniform/case6/fine-dining-host-06.jpg",
      description: "Elegant host and hostess uniform for premium restaurant service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Nightlife & Lounge Look 01",
      category: "Hotel Uniform",
      collection: "case7",
      image: "images/uniform/case7/nightlife-lounge-01.jpg",
      description: "Statement uniform for luxury lounge, bar and evening service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Nightlife & Lounge Look 02",
      category: "Hotel Uniform",
      collection: "case7",
      image: "images/uniform/case7/nightlife-lounge-02.jpg",
      description: "Statement uniform for luxury lounge, bar and evening service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Nightlife & Lounge Look 03",
      category: "Hotel Uniform",
      collection: "case7",
      image: "images/uniform/case7/nightlife-lounge-03.jpg",
      description: "Statement uniform for luxury lounge, bar and evening service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Nightlife & Lounge Look 04",
      category: "Hotel Uniform",
      collection: "case7",
      image: "images/uniform/case7/nightlife-lounge-04.jpg",
      description: "Statement uniform for luxury lounge, bar and evening service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Nightlife & Lounge Look 05",
      category: "Hotel Uniform",
      collection: "case7",
      image: "images/uniform/case7/nightlife-lounge-05.jpg",
      description: "Statement uniform for luxury lounge, bar and evening service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },
    {
      name: "Nightlife & Lounge Look 06",
      category: "Hotel Uniform",
      collection: "case7",
      image: "images/uniform/case7/nightlife-lounge-06.jpg",
      description: "Statement uniform for luxury lounge, bar and evening service.",
      specs: { Customization: "Design, fabric, colors & logo fully custom-made", MOQ: "50 pcs" },
    },

    {
      name: "Housekeeping Uniform 01",
      category: "Hotel Uniform",
      collection: "housekeeping",
      image: "images/uniform/housekeeping/housekeeping-01.jpg",
      description: "Ready-to-wear housekeeping uniform for guestroom care and public-area teams.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Housekeeping Uniform 02",
      category: "Hotel Uniform",
      collection: "housekeeping",
      image: "images/uniform/housekeeping/housekeeping-02.jpg",
      description: "Ready-to-wear housekeeping uniform for guestroom care and public-area teams.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Housekeeping Uniform 03",
      category: "Hotel Uniform",
      collection: "housekeeping",
      image: "images/uniform/housekeeping/housekeeping-03.jpg",
      description: "Ready-to-wear housekeeping uniform for guestroom care and public-area teams.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Housekeeping Uniform 04",
      category: "Hotel Uniform",
      collection: "housekeeping",
      image: "images/uniform/housekeeping/housekeeping-04.jpg",
      description: "Ready-to-wear housekeeping uniform for guestroom care and public-area teams.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Housekeeping Uniform 05",
      category: "Hotel Uniform",
      collection: "housekeeping",
      image: "images/uniform/housekeeping/housekeeping-05.jpg",
      description: "Ready-to-wear housekeeping uniform for guestroom care and public-area teams.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Housekeeping Uniform 06",
      category: "Hotel Uniform",
      collection: "housekeeping",
      image: "images/uniform/housekeeping/housekeeping-06.jpg",
      description: "Ready-to-wear housekeeping uniform for guestroom care and public-area teams.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Restaurant Uniform 01",
      category: "Hotel Uniform",
      collection: "restaurant",
      image: "images/uniform/restaurant/restaurant-01.jpg",
      description: "Ready-to-wear restaurant service uniform for polished front-of-house presentation.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Restaurant Uniform 02",
      category: "Hotel Uniform",
      collection: "restaurant",
      image: "images/uniform/restaurant/restaurant-02.jpg",
      description: "Ready-to-wear restaurant service uniform for polished front-of-house presentation.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Restaurant Uniform 03",
      category: "Hotel Uniform",
      collection: "restaurant",
      image: "images/uniform/restaurant/restaurant-03.jpg",
      description: "Ready-to-wear restaurant service uniform for polished front-of-house presentation.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Restaurant Uniform 04",
      category: "Hotel Uniform",
      collection: "restaurant",
      image: "images/uniform/restaurant/restaurant-04.jpg",
      description: "Ready-to-wear restaurant service uniform for polished front-of-house presentation.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Restaurant Uniform 05",
      category: "Hotel Uniform",
      collection: "restaurant",
      image: "images/uniform/restaurant/restaurant-05.jpg",
      description: "Ready-to-wear restaurant service uniform for polished front-of-house presentation.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
    {
      name: "Restaurant Uniform 06",
      category: "Hotel Uniform",
      collection: "restaurant",
      image: "images/uniform/restaurant/restaurant-06.jpg",
      description: "Ready-to-wear restaurant service uniform for polished front-of-house presentation.",
      specs: { Availability: "Standard styles and sizes", MOQ: "50 pcs" },
    },
  ],
};
